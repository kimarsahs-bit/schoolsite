...
export default function StudentAIAssistant() {
  ...
  const [isStreaming, setIsStreaming] = useState(false); // State to track if AI is responding
  ...
  const handleStopResponse = () => {
    if (abortControllerRef.current) {
      ...
      geminiService.stopResponse();
      setIsStreaming(false);
      setIsLoading(false);
      toast.success("AI response stopped.");
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    geminiService.setAbortController(controller);
    ...
    const sessionsWithAssistantMessage = updatedSessions.map(session =>
      session.id === currentSessionId
        ? {
            ...session,
    ...
    setSessions(sessionsWithAssistantMessage);

    try {
      // Use streaming to get faster responses
      let fullResponse = "";
      ...
      // Save final sessions with complete response
      setSessions(prevSessions => {
      ...
      // Check if error is due to user stopping the response
    ...
    <div className="flex h-screen bg-black">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-gray-900 border-b border-gray-700 p-4 flex items-center justify-between">
          {/* Left: Sidebar toggle button and Back button */}
            ...
            <div className="flex gap-2">
              ...
              {isStreaming && (
                <Button
                  ...
                  className="bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 text-white shrink-0 h-12 w-12 rounded-xl transition-all duration-300 hover:scale-110"
